...
user: "claude"
command: "/sp.tasks 001-todo-backend"
labels: ["todo-backend", "tasks", "fastapi", "sqlmodel", "neon"]
links:
  spec: null
  ticket: null
  adr: null
  pr: null
files:
  - "todo-backend/tasks.md"
tests: []
---

# Todo Backend Tasks Generation
...
Generate a detailed, sequential task breakdown for implementing the Todo App Backend. Each task must include:
- Clear task description
- Which agent to consult (if applicable)
- Files to create/modify
- Git commit message
- Dependencies on previous tasks

**IMPORTANT AGENT CONSULTATION RULES:**
- Agent files are located at: `.claude/agents/`
- Available agents:
  1. `fastapi-development-assistant` - For all FastAPI related tasks
  2. `neon-database-assistant` - For all NeonDB/PostgreSQL tasks
  3. `sqlmodel-development-assistant` - For all SQLModel/ORM tasks
- **MUST READ** the respective agent file BEFORE starting any task in their domain
- If stuck during ANY task, consult the relevant agent from `.claude/agents/`
- Do NOT hallucinate or make assumptions - always refer to agent documentation
...
### General Rules
1. **Always read the relevant agent file BEFORE starting a task**
2. **Agent location:** `.claude/agents/` directory
3. **Complete tasks sequentially** - respect dependencies
4. **Commit after EVERY task** with the specified commit message
5. **If stuck:** Consult the relevant agent from `.claude/agents/`
6. **Do not hallucinate** - refer to agent documentation

### Agent Consultation Matrix
| Task Domain | Agent to Consult |
|------------|------------------|
| FastAPI endpoints, routing, middleware | `fastapi-development-assistant` |
| Database connections, queries, NeonDB | `neon-database-assistant` |
| SQLModel models, ORM operations | `sqlmodel-development-assistant` |
| JWT authentication | `fastapi-development-assistant` |
| Pydantic schemas | `fastapi-development-assistant` |

### When Stuck
1. Identify the task domain
2. Open `.claude/agents/[relevant-agent]`
3. Read the agent's guidelines
4. Apply the recommendations
5. If still stuck, ask for clarification with agent context

## Success Criteria
- [ ] All 35+ tasks completed successfully
- [ ] Each task committed with appropriate message
- [ ] All three agents consulted where required
- [ ] All tests passing
- [ ] Code properly formatted with Ruff
- [ ] Documentation complete
- [ ] Backend runs successfully with `uv run dev`

## Final Verification Checklist
- [ ] Backend folder structure is clean and organized
- [ ] UV is used for all installations
- [ ] All dependencies installed correctly
- [ ] Database connection works with NeonDB
...
Generated comprehensive tasks.md file with detailed task breakdown for implementing the Todo App Backend. The file includes:

1. 11 phases with 33 specific tasks covering all aspects of backend development
2. Each task includes clear description, agent consultation requirements, files to create/modify, git commit messages, and dependencies
3. Proper sequential ordering with dependencies clearly marked
4. Parallel execution opportunities identified
5. All agent consultation rules properly documented (fastapi-development-assistant, neon-database-assistant, sqlmodel-development-assistant)
6. Implementation strategy from MVP to complete backend
7. Proper checklist format for all tasks

The tasks.md file is organized by phases following the user's requirements and is immediately executable by an LLM without additional context.